    ...
    def create_board(self):
        ...
        for row in range(ROWS):
            ...
            for col in range(COLS):
                if row == 0 or row == 7:
                    self.board[row].append(Piece(row, col, piece_types[col], "b" if row <= 1 else "w", self.screen))
                elif row == 1 or row == 6:
                    self.board[row].append(Piece(row, col, "pawn", "b" if row <= 1 else "w", self.screen))
                else:
    ...
    def draw_indexes(self, screen):
        for i in range(2):
            for row in range(ROWS):
                ...
                font = font_obj.render(str(ROWS - row), True, BLACK)
        ...
        for j in range(2):
            for col in range(COLS):
                font_obj = pygame.font.SysFont("arial black", 40)
                font = font_obj.render(chr(65 + col), True, BLACK)
        ...
        if piece.piece_type == "pawn":
            ...
            if piece.col > 0:
                if piece.row < 7 if piece.color == "b" else piece.row > 0:
                    if self.get_piece(piece.row - 1 if piece.color == "w" else piece.row + 1, piece.col - 1) and self.get_piece(piece.row - 1 if piece.color == "w" else piece.row + 1, piece.col - 1).color != piece.color:
            ...
            if not piece.moved:
                for i in range(2):
                    ...
                    if rook:
                        if not rook.moved:
                            col = rook.col
                            for _ in range(3 if rook.col == 0 else 2):
                                if rook.col == 0:
                                    col += 1
                                else:
                                    col -= 1
